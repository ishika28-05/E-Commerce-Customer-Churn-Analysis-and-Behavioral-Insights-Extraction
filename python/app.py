...
with col1:
    ...
    login_device = st.selectbox("Preferred Login Device", list(login_device_map.keys()))
with col2:
    ...
    payment_mode = st.selectbox("Preferred Payment Mode", list(label_encoders['PreferredPaymentMode'].classes_))
with col3:
    gender = st.selectbox("Gender", list(label_encoders['Gender'].classes_))
    order_category = st.selectbox("Prefered Order Cat", list(label_encoders['PreferedOrderCat'].classes_))
    marital_status = st.selectbox("Marital Status", list(label_encoders['MaritalStatus'].classes_))
    number_of_add = st.slider("Address Count", 1, 22, 15)
    order_amt_hike_from_last_yr = st.slider("Order Hike", 11, 26, 15)
